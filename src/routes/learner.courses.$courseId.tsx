...
function LearnerCourseNotebook() {
  ...
  const chatMutation = useMutation({
    mutationFn: (message: string) => chatWithCourse(courseId, message),
    onSuccess: (response) => {
      window.setTimeout(() => {
        setActiveSteps([]);
  ...
  function submitQuestion() {
    ...
    if (!trimmedQuestion || chatMutation.isPending || streamingAnswer) return;

    setMessages((current) => [
      ...current,
      { id: `${Date.now()}-student`, role: "student", text: trimmedQuestion },
    ]);
    setQuestion("");
                    ...
                    <div className="flex items-start gap-3">
                      <div
                        className={[
                          ...
                          isHighlighted ? "bg-amber-200 text-amber-700" : "bg-red-500/10 text-red-600",
                        ].join(" ")}
                      >
            ...
            <CardContent className="flex min-h-[590px] flex-col">
                  ...
                  <div className="grid min-h-[300px] place-items-center text-center">
                    <div>
                      ...
                      <p className="mt-1 max-w-md text-sm text-muted-foreground">
                        The AI tutor will search the course PDFs and answer using the most relevant
                        extracted chunks.
                      </p>
                    </div>
                  </div>
              ...
              <div className="mt-4 flex gap-3">
                ...
                <Button
                  ...
                  disabled={!question.trim() || chatMutation.isPending || Boolean(streamingAnswer)}
